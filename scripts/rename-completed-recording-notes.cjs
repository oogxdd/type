...
function isPlaceholderRecordingSuffix(value) {
...
function main() {
  ...
  const backupDir = path.join(
    root,
    ".migration-backups",
    `rename-completed-recordings-${stamp}`
  );
  ...
  const notes = walkMarkdown(root).map((filePath) => {
    ...
    const match = oldName.match(UTC_TIMESTAMP_FILE_NAME_RE);
    return {
      filePath,
      ...
      oldName,
      parsed,
      timestampPrefix: match ? match[1] : null,
  ...
  for (const [folder, entries] of folderEntries.entries()) {
    ...
    for (const entry of entries) {
      ...
      const status = String(meta.transcription_status || "").toLowerCase();
      if (type !== "audio_recording" || status !== "completed") {
        continue;
      }
      if (!entry.timestampPrefix || !isPlaceholderRecordingSuffix(entry.suffix)) {
        continue;
      }

      occupied.delete(entry.oldName.toLowerCase());

      const slug = buildSlug(entry.parsed.body, "recording");
      const base = `${entry.timestampPrefix}-${slug}`;
  ...
  const summary = {
    ...
    preview: renamePlan.slice(0, 50).map((item) => ({
      from: item.relFrom,
      to: item.relTo,
    })),
  };
  ...
  if (renamePlan.length === 0 && orderFiles.length === 0) {
    console.log(JSON.stringify(summary, null, 2));
    return;
  }
  ...
  for (let i = 0; i < renamePlan.length; i += 1) {
    ...
    const tempPath = `${item.from}.__completed_rec_rename_tmp_${i}`;
