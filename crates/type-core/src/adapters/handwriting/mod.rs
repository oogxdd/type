...
pub const HANDWRITING_FRONTMATTER_TYPE: &str = "handwriting_attachment";
...
#[derive(Deserialize)]
pub struct SaveHandwritingAttachmentArgs {
...
pub struct HandwritingAttachmentWriteResult {
...
pub struct QueueHandwritingOcrArgs {
...
pub struct HandwritingOcrQueueResult {
    pub scanned: usize,
    pub queued: usize,
    pub skipped: usize,
    pub in_flight: usize,
...
pub struct HandwritingOcrQueueSnapshot {
    pub running: bool,
...
pub struct HandwritingOcrListItem {
    pub note_path: String,
    pub folder_path: String,
...
pub struct HandwritingOcrListResult {
    pub queue: HandwritingOcrQueueSnapshot,
    pub jobs: Vec<HandwritingOcrListItem>,
}
...
pub struct QueuedHandwritingOcrJob {
    pub note_rel: String,
...
pub struct HandwritingNoteInfo {
    pub note_rel: String,
    pub note_path: PathBuf,
    pub attachment_rel: String,
    pub attachment_path: PathBuf,
    pub status: String,
    pub error: Option<String>,
    pub updated_ms: Option<i64>,
...
pub struct HandwritingOcrQueueState {
    pub running: bool,
    pub current_note: Option<String>,
    pub pending: VecDeque<QueuedHandwritingOcrJob>,
    pub known_notes: HashSet<String>,
}

/// Core handwriting gateway. It owns attachment persistence and the
/// process-global OCR queue.
pub struct HandwritingAdapter {
    app: AppEnv,
}
...
pub fn handwriting_ocr_queue_state() -> &'static Mutex<HandwritingOcrQueueState> {
...
pub fn active_handwriting_note_paths() -> HashSet<String> {
    let queue = handwriting_ocr_queue_state();
...
pub fn handwriting_queue_snapshot() -> HandwritingOcrQueueSnapshot {
    let queue = handwriting_ocr_queue_state();
    let state = queue.lock().expect("handwriting ocr queue poisoned");
    let pending = state
        .pending
...
pub fn normalize_image_extension(value: &str) -> Option<&'static str> {
...
pub fn supported_image_extension(
...
pub fn collect_handwriting_notes(root: &Path) -> Result<Vec<HandwritingNoteInfo>, String> {
...
pub fn update_handwriting_note_status(
    note_path: &Path,
    status: &str,
    error: Option<String>,
...
pub fn spawn_handwriting_ocr_worker_if_needed() {
    let should_spawn = {
        let queue = handwriting_ocr_queue_state();
        let mut state = queue.lock().expect("handwriting ocr queue poisoned");
...
pub fn handwriting_initial_body() -> String {
    String::new()
}

/// Generate a unique filename for a handwriting note.
pub fn handwriting_note_file_name(
    folder: &Path,
    timestamp_ms: i64,
    note_id: &str,
