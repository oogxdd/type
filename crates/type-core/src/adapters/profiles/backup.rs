...
fn documents_dir(app: &AppEnv) -> Result<PathBuf, String> {
    if let Some(path) = app.documents_dir.clone() {
        return Ok(path);
    }
...
pub fn create_profiles_backup_zip_impl(
    app: &AppEnv,
...
pub fn export_profiles_to_documents_impl(
    app: &AppEnv,
