...
/// Core security gateway. All app-data and runtime-key handling stays
...
impl SecurityGateway for SecurityAdapter {
...
fn security_file_path(app: &AppEnv) -> Result<PathBuf, String> {
...
fn read_security_config(app: &AppEnv) -> Result<SecurityConfigFile, String> {
...
fn ensure_security_runtime_loaded(app: &AppEnv) -> Result<(), String> {
...
fn security_state_snapshot(app: &AppEnv) -> Result<SecurityState, String> {
...
pub fn decrypt_note_body_for_read(body: &str) -> Result<String, String> {
    if !is_encrypted_note_body(body) {
        return Ok(body.to_string());
    }
    let runtime = security_runtime_state();
...
pub fn encrypt_note_body_for_write(body: &str) -> Result<String, String> {
    let runtime = security_runtime_state();
    let state = runtime
        .lock()
        .map_err(|_| "Security runtime lock poisoned.".to_string())?;
...
pub fn ensure_security_unlocked_for_app(app: &AppEnv) -> Result<(), String> {
    ensure_security_runtime_loaded(app)?;
    let runtime = security_runtime_state();
    let state = runtime
        .lock()
...
fn panic_reset_local_data(app: &AppEnv) -> Result<(), String> {
...
pub fn lock_security_impl(app: &AppEnv) -> Result<SecurityState, String> {
    ensure_security_runtime_loaded(app)?;
    let runtime = security_runtime_state();
    let mut state = runtime
        .lock()
...
pub fn set_security_preferences_impl(
    app: &AppEnv,
