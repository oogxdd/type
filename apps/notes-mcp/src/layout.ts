...
export type MemoryArea = 'me' | 'agent' | 'reviews';
export type ResolvedLayout = {kind: 'legacy' | 'system'; stream: string; me: string; agent: string; reviews: string};
const layouts: Record<'legacy' | 'system', ResolvedLayout> = {
  legacy: {kind: 'legacy', stream: 'Feed', me: 'me', agent: 'agent', reviews: 'reviews'},
  system: {kind: 'system', stream: '_system/stream', me: '_system/me', agent: '_system/agent', reviews: '_system/reviews'},
};
...
export async function resolveLayout(root: string, mode: LayoutMode = 'auto'): Promise<ResolvedLayout> {
  ...
  const legacy = (await Promise.all(['Feed', 'me', 'agent', 'reviews'].map(exists))).some(Boolean);
  const system = (await Promise.all(['_system/stream', '_system/me', '_system/agent', '_system/reviews'].map(exists))).some(Boolean);
