...
export function createServer(repository: NotesRepository) {
  ...
  server.registerTool('prepare_context', {description:'Start a personal session: full filtered agent instruction documents, overview, corrections, preferences, handoff and paginated source references. Read instructionDocuments before proceeding. No model runs here.',inputSchema:{mode:z.enum(['review','morning_note','observation','conversation']).default('review'),...page},annotations},
