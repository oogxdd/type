...
export class DocumentSession {
  ...
  async load(path: string, refresh = false): Promise<void> {
    ...
    entry.error = null;
    this.notify();
    const read = this.io.read(path).then((content) => {
      if (!this.closed && !entry.dirty && !entry.saving && entry.version === version) {
        entry.content = content;
        entry.loaded = true;
      }
    }).catch((error: unknown) => {
      if (!this.closed && entry.version === version) entry.error = getErrorMessage(error);
    }).finally(() => {
  ...
  private leave(path: string): Promise<void> {
    ...
    const departure = (async () => {
      ...
      const entry = this.documents.get(path);
      if (this.closed || this.selected.has(path) || !entry?.loaded) return;
      // A move/deletion may already have removed this file. Do not recreate it.
      try { await this.io.read(path); } catch { this.documents.delete(path); return; }
