...
| `object_sync.rs`| bucket status/settings/test, sync now, request sync, GC, enable/unlock encryption, pairing link |
...
- **object_sync** — sync through a user-supplied **S3-compatible bucket** (R2 / B2 / S3 / MinIO), the automatic cross-network transport. A folder module: `s3.rs` (hand-rolled SigV4 over blocking `reqwest`, checked against the AWS test-suite vectors), `manifest.rs` (scan the notes root; device-local `.type/object-sync-state.json`), `engine.rs` (one round: scan → fetch → three-way diff → apply → publish), `codec.rs` (`PlaintextCodec` / `EncryptedCodec` — how keys and payloads are formed), `crypto.rs` (Argon2id-wrapped random vault key, HKDF subkeys, XChaCha20-Poly1305), `pairing.rs` (`type2://cloud/...` QR), `scheduler.rs` (debounce / min gap / backoff / idle poll on a `std::thread`, since the core has no tokio on mobile). Blobs are **content-addressed and immutable** and each device writes **only its own manifest**, which removes the concurrent-sync lost-update race without needing conditional writes. Entries carry a **revision**, not a trusted timestamp — see `docs/OBJECT_SYNC.md`. Conflicts produce the same `.conflict.md` siblings the git transport does. Credentials and the vault key live in the device-local `.type/device.json`, never in the bucket and never in synced settings.
...
- **Two encryption layers, different jobs**: the app lock (`.notes-security.json`) encrypts note *bodies* on local disk and leaves front-matter and filenames plaintext by design; recordings and attachments are not covered. Object sync's end-to-end encryption is separate and applies only to what leaves the device — whole files, opaque object keys, encrypted manifests, attachments included. They compose; neither replaces the other. Object sync's vault key sits beside the bucket credentials in `.type/device.json`, so it defends against the storage provider, not against someone holding the device.
- **Object sync and git sync can both be on**: they are independent transports over the same notes root. `.type/object-sync-state.json` is excluded from git for the same reason `device.json` is.
