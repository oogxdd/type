...
- **recording** — `components/recording-note-header`; `hooks/{recordings-context,
  ...
  auto-queues to whichever backend `AppConfig.transcription_provider` names — local
  Whisper (default) or AssemblyAI. Queue bookkeeping uses the shared `processing`
  hooks above.
...
- **Home owns the menu/capture gesture.** `apps/mobile/src/screens/home-screen.tsx` mounts both persistent layers inside one `Home` route, with native-stack gestures disabled for that route. One memoized pan classifies direction and owns release; native scroll views run simultaneously and supply bottom overscroll. Do not reintroduce a pushed Capture route, native full-screen back recognizer, independent horizontal pans, or the old height split. `apps/mobile/GESTURES.md` documents the thresholds and cancellation rules.
- **Capture-screen gestures must stay memoized.** `GestureDetector` re-runs `updateAttachedGestures` on every render (its effect depends on `props`), so an unmemoized `Gesture.*` re-serializes its whole closure graph into the UI runtime on every keystroke — including mid-commit, while the spring still holds the callback it was serialized with. Anything a gesture worklet calls needs one identity for the life of the screen (the commit ref proxy in `capture-screen.tsx`), and window dimensions belong in shared values, not captured props. On the new architecture the UI runtime is the iOS main thread, so a throw there is an uncatchable `abort`, not a caught JS error.
...
- **Mobile auto-sync timing is by kind, not one delay.** Typing (`"edit"`) waits for a 45 s pause (3 min ceiling); finished actions (filed, closed, deleted, moved, audio) sync after 1.5 s; foregrounding syncs now. On backgrounding, drafts are flushed and owed changes synced inside an iOS background task started natively (`modules/background-task`). Pass the right `SyncTiming` to `scheduleAutoSync`. See [docs/SYNC_TIMING.md](docs/SYNC_TIMING.md).
...
- **Two different transcription settings.** The per-folder `transcription_mode` above is *synced* and answers "which device transcribes this folder's recordings". The desktop's own backend choice is `AppConfig.transcription_provider` (`whisper` | `assemblyai`) in the **device-local** `config.json` — it never syncs, and an unrecognized or absent value falls back to local Whisper, so an install that predates the setting is never silently switched to uploading audio. Both the bulk queue and the single-note retry read it; the AssemblyAI key is the same one the mobile config uses.
- **Ship the release core.** `codegen:ios:device` builds Cargo's *debug* profile — the Rust code and the libgit2 C inside it unoptimized, a ~700 MB static library. It is for developing on a phone. Anything that ships (ad-hoc, TestFlight, CI) uses `codegen:ios:release`; every iOS build up to 0.4.1 went out with the debug core, and reading all previews was ~3× slower for it. Switching from the dev variant back to a release build means regenerating the core, not just re-prebuilding.
...
- **An Iroh handler must not return while the peer still needs the connection.** iroh's `Router` drops the `Connection` the moment `ProtocolHandler::accept` returns, and a dropped QUIC connection discards anything it has not had acknowledged — `SendStream::finish` marks the end of a stream, it does not wait for delivery. Replying and returning therefore races the reply away, which is what produced "the desktop did not acknowledge Iroh pairing: connection lost" and intermittently truncated syncs. `serve_iroh_connection` loops on `accept_bi()` until the peer closes; the phone keeps one connection per computer and multiplexes streams on it. Loopback tests cannot catch a regression here by asserting delivery (the racing close nearly always loses locally) — assert that one connection serves several streams. See `docs/IROH_SYNC_EXPERIMENT.md` § Connection rules.
...
- **Shelved work lives in tags, not branches.** Anything abandoned-but-worth-keeping
  ...
  See [docs/ARCHIVED_WORK.md](./docs/ARCHIVED_WORK.md).
- **Git sync uses libgit2**, not shell git. The Rust backend handles all git operations.
- **Audio history migration handoff**: [docs/AUDIO_HISTORY_MIGRATION.md](docs/AUDIO_HISTORY_MIGRATION.md) records the user’s selected in-place `.git` replacement plan, production notes path, verified preview copy, and pending device migration steps. Read it before helping with this migration; the preview is not activated, and old phone repositories must not sync into the rewritten history.
...
- **Note previews are persisted per profile** in localStorage (`notes-viewer-note-previews-v1:<profileId>`) and hydrated on launch for an instant first paint, then revalidated via the bulk `list_note_previews` command (stale-while-revalidate). Persistence is disabled while encryption is on, and enabling encryption purges the snapshots (`clearPersistedNotePreviews` in `shared/lib/storage`). `get_tree` never reads note bodies — Feed is name-sorted (file names are time-prefixed) and the UI re-sorts by front-matter timestamps from previews. The phone instead keeps one preview snapshot per working folder under its app data and re-reads only notes whose `NoteEntry.version` (mtime/ctime/size from the stat `get_tree` already does) changed; `list_note_previews` takes that version *before* reading, never after. The full cache/invalidation design (and the deferred TanStack Query decision) is documented in `docs/architecture/07-frontend-caching.md`.
