...
There are **two apps over one Rust core**: the desktop app is Tauri v2 + React (macOS/Windows/Linux), and the mobile app is React Native (Expo), talking to the same core through UniFFI bindings. A third, much smaller shell — `crates/type-tui` — drives the same core from the terminal.
...
crates/type-tui/       Terminal shell over type-core (ratatui): tree/list/editor,
                       vim-like keys, `:` commands, git sync, and a split-or-
                       nested note list mirroring the desktop. See its README.
...
- **`type-core` is feature-gated**: `notes`, `profiles` and `security` always compile; `git-sync`, `local-sync`, `recordings`, `handwriting`, `import` and `profile-backup` are cargo features. `default = ["full"]`, so the desktop and FFI shells are unaffected — but a **new adapter/application/ports module must be added to the matching `#[cfg(feature = …)]` list** in `adapters/mod.rs`, `application/mod.rs` and `ports/mod.rs`, and any heavy dependency marked `optional = true`. Verify both ends: `cargo check -p type-core` and `cargo check -p type-core --no-default-features`.
- **Two shells, one core** (three counting the TUI): new backend features go in `crates/type-core`, then get exposed twice — a `#[tauri::command]` in `apps/desktop/src-tauri/src/commands/` and a `#[uniffi::export]` in `crates/type-ffi`. After changing `type-ffi`, regenerate the mobile native module (Mac, ubrn) and keep `packages/mobile-core/src/raw-core.ts` in sync. Step-by-step checklist (and what the ubrn codegen actually does): `docs/architecture/09-adding-features-and-codegen.md`.
- **The TUI repaints only when something changed.** `crates/type-tui/src/main.rs` guards `terminal.draw` behind a dirty flag, because `CrosstermBackend::draw` emits colour/attribute resets even when no cell differs — drawing on every 50ms poll made an idle app stream escape sequences, which terminals read as a busy job (iTerm2 puts a spinner on the tab). Anything new that changes what is on screen has to set that flag.
