...
> **Внимание (2026-07-11):** desktop-фронтенд пережил большой рефакторинг:
> feature-slices и дерево React-провайдеров заменены на слои по типам
> (`components/` / `state/` / `hooks/` / `lib/` / `api/`) с zustand-сторами и
> plain-function действиями. Примеры путей и структура фронтенда в главах
> могли устареть — актуальное описание в [AGENTS.md](../../AGENTS.md);
> глава 07 обновлена. Rust-слои (domain/application/ports/adapters) не менялись.
