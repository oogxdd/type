# Предлагаемый общий state-слой: полный API

[Обзор](../12-architecture-map.md) · [Каталог](../14-exposed-api-catalog.md) · [Mermaid-исходник](./proposed-full-api.mmd)
...
flowchart LR
    ...
    Shared["Предлагаемый общий код stores<br/><b>NotesStore</b><br/>loadNote · saveNote · cache<br/><b>ProfilesStore</b><br/>load · switchProfile · updateSettings<br/><b>SecurityStore</b><br/>refresh · lock · unlock<br/><b>TagRegistryStore</b><br/>load · save<br/><b>GitSyncStore</b><br/>refreshStatus · pull · push<br/><br/>createStores(api)<br/>Отдельный экземпляр в каждом приложении"]
    Core -->|Tauri commands / adapter| Shared
    Core -->|FFI / adapter| Shared
    Shared --> Desktop
    Shared --> Mobile
    Desktop["Desktop / React<br/>Платформенный UI и lifecycle<br/>set_app_icon · D / D"]
    Mobile["Mobile / React Native<br/>Платформенный UI и lifecycle<br/>init_core · M / M"]
    Legend["D: desktop · M: mobile · DM: обе<br/>API: доступно · Вызовы: ссылки в frontend<br/>—: не найдено; не runtime-трассировка<br/>Подгруппы по назначению<br/>Предложение; API и ссылки по текущему коду"]
```
